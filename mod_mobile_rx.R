...
mobile_rx_server <- function(id, pool, current_pt, user_info,
                             ...
                             email_enabled  = FALSE,       # send .xlsx as email attachment → PAD trigger
                             smtp_from      = "",
                             smtp_to        = "",
                             smtp_pass      = "",
  ...
  moduleServer(id, function(input, output, session) {
    ...
    # Send prescription .xlsx as email attachment via Gmail SMTP.
    # PAD monitors the inbox and triggers on subject "RenalIQ Prescription".
    # Uses only base R + curl (available on all platforms) — no extra packages needed.
    send_rx_email <- function(xlsx_path, fname, pt) {
      if (!isTRUE(email_enabled)) return(invisible(NULL))
      if (!file.exists(xlsx_path))  return(invisible(NULL))
      if (!requireNamespace("blastula", quietly = TRUE)) {
        message("Email: blastula package not installed — skipping email notification.")
      ...
      tryCatch({
        email <- blastula::compose_email(
          body = blastula::md(paste0(
            "**RenalIQ Prescription Export**\n\n",
            "Patient: ", pt_name, "\n",
            "Date: ", as.character(Sys.Date()), "\n\n",
            "Prescription file attached. This email is auto-generated by RenalIQ."
          ))
        ) |>
          blastula::add_attachment(file = xlsx_path, filename = fname)

        # Gmail App Password auth — creds_envvar reads SMTP_PASS from environment
        Sys.setenv(SMTP_PASS = smtp_pass)   # ensure env var is current
        blastula::smtp_send(
          email   = email,
          from    = smtp_from,
          to      = smtp_to,
          subject = paste0("RenalIQ Prescription — ", pt_name, " — ", as.character(Sys.Date())),
        ...
        message("Email: prescription sent to ", smtp_to)
      }, error = function(e) {
    ...
    observeEvent(input$save_rx, {
      ...
      if (nrow(final_df) > 0) {
        ...
        for (i in seq_len(nrow(final_df))) {
          r <- final_df[i, ]
          # In-memory existence check using the same columns as the DB unique
          # constraint: (brand_name, generic, dose, freq, route) — no duration.
          already_exists <- nrow(master[
            tolower(trimws(master$brand_name)) == tolower(trimws(r$brand_name)) &
            tolower(trimws(master$generic))    == tolower(trimws(r$generic))    &
            tolower(trimws(master$dose))       == tolower(trimws(r$dose))       &
            tolower(trimws(master$freq))       == tolower(trimws(r$freq))       &
            tolower(trimws(master$route))      == tolower(trimws(r$route)), ]) > 0
          if (!already_exists) {
            dbExecute(pool,
              "INSERT INTO drug_master (brand_name, generic, dose, freq, route, duration)
               VALUES ($1, $2, $3, $4, $5, $6)
      ...
      # ── Export: local folder or GCS, then email notification ────────────
      tryCatch({
        result <- export_rx(final_df, current_pt())
        if (!is.null(result)) {
          showNotification(paste0("📤 ", result), type = "message", duration = 6)
          # Send email with the xlsx attached so PAD can trigger on it
          pt_name <- trimws(paste(current_pt()$first_name %||% "",
                                  current_pt()$last_name  %||% ""))
          fname   <- paste0("Rx_", gsub("[^A-Za-z0-9]", "_", pt_name),
                            "_", as.character(Sys.Date()), ".xlsx")
          # For local export the file already exists at dest; for GCS re-create temp
          local_path <- if (nchar(rx_output_dir) > 0 && dir.exists(rx_output_dir))
            file.path(rx_output_dir, fname) else {
              tmp2 <- tempfile(fileext = ".xlsx")
              export_rows2 <- lapply(seq_len(nrow(final_df)), function(i) {
                r <- final_df[i, ]
                dp   <- parse_dose_parts(r$dose)
                durp <- parse_duration_parts(r$duration)
                data.frame(Brand_Name=r$brand_name, Generic=r$generic,
                           Dose=r$dose, Dose_Qty=dp$qty, Dose_Unit=dp$unit,
                           Freq_Code=r$freq, Freq_Display=map_freq_display(r$freq),
                           Route=r$route, Route_Display=map_route_display(r$route),
                           Duration=r$duration, Duration_Qty=durp$qty,
                           Duration_Type=durp$type, stringsAsFactors=FALSE)
              })
              writexl::write_xlsx(list(Prescription=do.call(rbind, export_rows2)), tmp2)
              tmp2
            }
          send_rx_email(local_path, fname, current_pt())
        }
      }, error = function(e) {
